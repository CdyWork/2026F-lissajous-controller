...
**V2（第五问控制链路打通）**：在 V1 基础上增加视觉探测模式 `PROBE`，
STM32 通过 USART3（PB10/PB11）接收树莓派命令，再经 SPI 控制 FPGA 输出
10 ms 周期探测锯齿波。支持 100/200/500/1000/2000/5000 us 六档有效斜坡，
覆盖 1 kHz～100 kHz 的视觉粗测频。串口状态回读协议为 `VER=3`，保留
USART1/COM9 镜像调试输出。当前已验证 Pi->STM32->FPGA 切档链路；摄像头
采集是否可用取决于 OV5647/CSI 实物状态。
