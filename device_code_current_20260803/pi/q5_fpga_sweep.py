    ...
    with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as listener:
        ...
        try:
            with Uart(args.serial) as uart, Camera(args.device, args.capture_fps) as camera:
                ...
                # Stay in this process while the scope/camera settles. This
                # sends IDLE only once instead of creating a systemd restart
                # loop that repeatedly blanks the oscilloscope output.
                idle_cache = prepare_idle_cache(
                    uart, camera, retry_until_valid=True
                )
                phase_servo = None
                    ...
                    if listener in ready:
                        ...
                        with connection, connection.makefile("rwb") as stream:
                            ...
                            try:
                                ...
                                # A timed-out diagnostic client must not kill
                                # the camera/UART owner and restart with IDLE.
                                print(json.dumps({
                                    "service_client_disconnected": str(error),
                                    "command": command,
                                }), flush=True)
