...
def start_q5_phase_feedforward(
    ...
    probe = observe(
        ...
        # The live branch comparison below now corrects an ambiguous probe,
        # so waiting a full 1.6 s for this unsigned ellipse is unnecessary.
        max_observation_s=1.35,
    )
    ...
    def try_candidate(
        ...
        return {
            ...
            "physical_drift_rate_degrees_per_s": candidate_physical_drift,
            "timing": candidate_timing,
        }

    def refine_candidate(
        trial: dict[str, object], label_prefix: str
    ) -> dict[str, object]:
        servo = trial["servo"]
        coarse = trial["observation"]
        assert isinstance(servo, Q5PhaseFeedForward)
        assert isinstance(coarse, dict)
        coarse_error = float(coarse["error_degrees"])
        fine_candidates: list[dict[str, object]] = []
        errors: list[str] = []
        candidates_with_reference = [
            (
                coarse,
                servo.reference_phase_degrees,
                servo.reference_time_s,
                0.0,
            )
        ]

        if coarse_error > FINE_LOCK_ERROR_DEGREES:
            coarse_reference_phase = servo.reference_phase_degrees
            coarse_reference_time_s = servo.reference_time_s
            for direction in (-1.0, 1.0):
                try:
                    now = time.monotonic()
                    compensated_base = (
                        coarse_reference_phase
                        + servo.compensation_rate_degrees_per_s
                    ...
                    reference_time_s = time.monotonic()
                    servo.reference_phase_degrees = candidate_phase
                    servo.reference_time_s = reference_time_s
                    servo.last_sent_millidegrees = (
                        int(round(candidate_phase * 1000.0)) % 360000
                    )
                    servo.next_due_s = reference_time_s + PHASE_UPDATE_INTERVAL_S
                    sample = observe(
                        candidate_phase,
                        f"{label_prefix}_refine_{'minus' if direction < 0.0 else 'plus'}",
                        expected_errors=(0.0,),
                    ...
                    delta = direction * coarse_error
                    candidates_with_reference.append(
                        (sample, candidate_phase, reference_time_s, delta)
                    )
                    sample_error = float(sample["error_degrees"])
                    if sample_error <= ZERO_REFINE_ACCEPT_DEGREES:
                        break

                    # If the first correction clearly improved the trace, its
                    # direction is now known. Continue once by the measured
                    # residual instead of wasting a frame on the opposite side.
                    if sample_error + VISUAL_PHASE_IMPROVEMENT_DEGREES < coarse_error:
                        follow_phase = set_phase(
                            uart,
                            servo.phase_at() + direction * sample_error,
                        )
                        follow_time_s = time.monotonic()
                        servo.reference_phase_degrees = follow_phase
                        servo.reference_time_s = follow_time_s
                        servo.last_sent_millidegrees = (
                            int(round(follow_phase * 1000.0)) % 360000
                        )
                        servo.next_due_s = follow_time_s + PHASE_UPDATE_INTERVAL_S
                        follow = observe(
                            follow_phase,
                            f"{label_prefix}_refine_follow",
                            expected_errors=(0.0,),
                            max_observation_s=ZERO_REFINE_MAX_OBSERVATION_S,
                            min_observation_s=ZERO_REFINE_MIN_OBSERVATION_S,
                            expected_tolerance_degrees=ZERO_REFINE_ACCEPT_DEGREES,
                        ...
                        fine_candidates.append(follow)
                        candidates_with_reference.append(
                            (
                                follow,
                                follow_phase,
                                follow_time_s,
                                delta + direction * sample_error,
                            )
                        )
                        break
        ...
        if (
            servo.reference_phase_degrees != best_phase
            or servo.reference_time_s != best_time_s
        ):
            now = time.monotonic()
            best_phase = set_phase(
                uart,
                best_phase
                + servo.compensation_rate_degrees_per_s * (now - best_time_s),
            )
            best_time_s = time.monotonic()
            servo.reference_phase_degrees = best_phase
            servo.reference_time_s = best_time_s
            servo.last_sent_millidegrees = (
                int(round(best_phase * 1000.0)) % 360000
            )
            servo.next_due_s = best_time_s + PHASE_UPDATE_INTERVAL_S
        return {
            "trial": trial,
            "servo": servo,
            "final": best_sample,
            ...
            "fine_probe": {
                "strategy": "directional_live_refinement",
                "coarse_error_degrees": coarse_error,
                "selected_delta_degrees": best_delta,
                "selected_error_degrees": float(best_sample["error_degrees"]),
                "attempt_errors": errors,
            },
        }

    # Refine the most likely branch first. Most measurements now need only one
    # extra frame. The mirrored zero-phase hypothesis is acquired only when the
    # primary branch still cannot meet the actual acceptance threshold.
    candidate_trials = [try_candidate(1.0, "final_primary")]
    branch_results = [refine_candidate(candidate_trials[0], "primary")]
    if float(branch_results[0]["final"]["error_degrees"]) > LOCK_ACCEPT_ERROR_DEGREES:
        candidate_trials.append(try_candidate(-1.0, "final_mirrored"))
        branch_results.append(refine_candidate(candidate_trials[1], "mirrored"))

    selected_result = min(
        branch_results,
        key=lambda result: float(result["final"]["error_degrees"]),
    )
    selected_trial = selected_result["trial"]
    feedforward = selected_result["servo"]
    final = selected_result["final"]
    fine_candidates = selected_result["fine_candidates"]
    fine_probe_report = selected_result["fine_probe"]
    assert isinstance(selected_trial, dict)
    assert isinstance(feedforward, Q5PhaseFeedForward)
    assert isinstance(final, dict)
    assert isinstance(fine_candidates, list)
    assert isinstance(fine_probe_report, dict)
    predicted_error = float(selected_trial["predicted_error_degrees"])
    selected_physical_drift = float(
        selected_trial["physical_drift_rate_degrees_per_s"]
    )
    candidate_timing = selected_trial["timing"]
    assert isinstance(candidate_timing, dict)

    # A mirrored fallback leaves that branch active. Restore the actually
    # selected modeled phase if the primary branch remained better.
    if selected_result is not branch_results[-1]:
        restored_phase = set_phase(uart, feedforward.phase_at())
        restored_at_s = time.monotonic()
        feedforward.reference_phase_degrees = restored_phase
        feedforward.reference_time_s = restored_at_s
        feedforward.last_sent_millidegrees = (
            int(round(restored_phase * 1000.0)) % 360000
        )
        feedforward.next_due_s = restored_at_s + PHASE_UPDATE_INTERVAL_S
    ...
    return feedforward, {
        ...
        "first": {
            ...
            **candidate_timing,
            ...
            "candidate_branch": float(selected_trial["sign"]),
            "candidate_branches": [
                {
                    "sign": float(candidate["sign"]),
                    "label": str(candidate["label"]),
                    "predicted_error_degrees": float(
                        candidate["predicted_error_degrees"]
                    ),
            ...
            "candidates": [
                candidate["observation"] for candidate in candidate_trials
            ],
            "fine_candidates": fine_candidates,
